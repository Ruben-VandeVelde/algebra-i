...
theorem l1_1_3_i (f : G) (h1 : ∀ g, f * g = g) (_h2 : ∀ g, g * f = g) : f = 1 := by
...
theorem l1_1_5_i (x y z : G) : x * y = x * z → y = z := by
  intro h
  rwa [← mul_left_cancel_iff]

theorem l1_1_5_ii (x y z : G) : x * z = y * z → x = y := by
  intro h
...
-- TODO: AddZeroClass short-circuit in Data/Int/Basic.lean?
def l1162 (n : ℤ) : AddCommGroup <| Mulint n where
  add := Mulint.add n
...
def l1166 : CommGroup TrivialGroup where
  ...
  mul _ _ := TrivialGroup.One
  inv _ := TrivialGroup.One
  mul_assoc a b c := Subsingleton.elim _ _
  one_mul a := Subsingleton.elim _ _
  mul_one a := Subsingleton.elim _ _
  mul_left_inv a := Subsingleton.elim _ _
  mul_comm a b := Subsingleton.elim _ _

def l1167 (K V : Type*) [Field K] [AddCommMonoid V] [Module K V] : AddCommGroup V :=
...
def l1168'' (K n : Type*) [CommSemiring K] [Subsingleton n] [Fintype n] :
...
def l1169 (α : Type*) (X : Set α) : Group (Equiv.Perm X) :=
  inferInstance
...
theorem ex_1_1_1 {G : Type*} [Group G] (x y : G) : (x * y)⁻¹ = y⁻¹ * x⁻¹ :=
  mul_inv_rev _ _
...
theorem mul_def (a b : { z : ℂ // ‖z‖ = 1 }) :
    a * b = ⟨a * b, by rw [norm_mul, a.prop, b.prop, one_mul]⟩ :=
  rfl

nonrec def mul_assoc (a b c : { z : ℂ // ‖z‖ = 1 }) : a * b * c = a * (b * c) := by
...
noncomputable def ex113 : Group { z : ℂ // ‖z‖ = 1 } where
  mul := (· * ·)
  mul_assoc := mul_assoc
  one := 1
...
theorem ex1_1_3' : {z : ℂ | ‖z‖ = 1} = { (Complex.exp (θ * Complex.I)) | (θ : ℝ) } := by
  ext z
