...
def l122Ba {G : Type _} [Group G] (H : Subgroup G) : Group H :=
  inferInstance

theorem l1_2_2_ba' {G : Type _} [Group G] (H : Subgroup G) :
...
def l122Ab {G : Type _} [Group G] (H : Set G) [Group H]
    (hmul : ∀ a b : H, ((a * b : H) : G) = (a * b : G)) (h1 : ((1 : H) : G) = 1) : Subgroup G
    where
  carrier := H
...
theorem l1_2_2_bc {G : Type _} [Group G] (H : Subgroup G) :
...
def l122Cb {G : Type _} [Group G] (H : Set G) (hne : H.Nonempty)
...
def ex1231 {G : Type _} [Group G] : Subgroup G
    where
  carrier := {1}
  mul_mem' {a b} ha hb := by
...
theorem l1_2_3_2 (n : ℤ) : (ex1232 n : Set ℤ) = {k : ℤ | n ∣ k} :=
  by
  ext
  simp [ex1232, Int.mem_zmultiples_iff]

def ex1233 (K : Type _) [Field K] (n : ℕ) : Subgroup (Matrix.GeneralLinearGroup (Fin n) K)
    where
...
def l124 {G : Type _} [Group G] (S : Set <| Subgroup G) : Subgroup G :=
  sInf S

theorem l1_2_4' {G : Type _} [Group G] (S : Set <| Subgroup G) :
    (l124 S : Set G) = ⋂ s ∈ S, (s : Set G) :=
  Subgroup.coe_sInf _

def d125 {G : Type _} [Group G] (X : Set G) : Subgroup G :=
  Subgroup.closure X

theorem l1_2_5' {G : Type _} [Group G] (X : Set G) :
    (Subgroup.closure X : Set G) = ⋂ H' ∈ {H : Subgroup G | X ⊆ H}, (H' : Set G) :=
  rfl

def n125Set {G : Type _} [Group G] (X : Set G) : Set (List G) :=
  {m : List G | ∀ x ∈ m, x ∈ X ∪ X⁻¹}

/- ./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(list.prod x) | x «expr ∈ » n1_2_5_set[n1_2_5_set] X} -/
def n125 {G : Type _} [Group G] (X : Set G) : Subgroup G
    where
...
-- 2019-2020
theorem n1_2_5' {G : Type _} [Group G] (X : Set G) :
    (Subgroup.closure X : Set G) = { (List.prod x) | x ∈ n125Set X } :=
  by
...
def l128A {G : Type _} [Group G] (H : Subgroup G) : Setoid G :=
  QuotientGroup.leftRel H

theorem l1_2_8_b {G : Type _} [Group G] [Fintype G] (H : Subgroup G) [Fintype H] :
    Fintype.card H ∣ Fintype.card G :=
  Subgroup.card_subgroup_dvd_card H

open scoped Cardinal

theorem l1_2_9 {G : Type _} [Group G] (H : Subgroup G) :
...
def d1210 {G : Type _} [Group G] (H : Subgroup G) :=
  #(G ⧸ H)

theorem d1_2_10' {G : Type _} [Group G] [Fintype G] (H : Subgroup G) :
...
/- ./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr * »(g, b)) | b «expr ∈ » B} -/
/- ./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr * »(«expr ⁻¹»(g), a)) | a «expr ∈ » A} -/
theorem n_1_2_13 {G : Type _} [Group G] (A B : Set G) (g : G) :
    A = { (g * b) | b ∈ B } ↔ { (g⁻¹ * a) | a ∈ A } = B :=
  by constructor <;> · rintro rfl; ext; simp

def ex124' {G : Type _} [Group G] (x : G × G) :=
  x.1 * x.2

/- ./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(ex_1_2_4' x) | x «expr ∈ » [finset.product/multiset.product/set.prod/list.product](A, B)} -/
theorem ex_1_2_4 {G : Type _} [Group G] (A B : Set G) :
  #({ (ex124' x) | x ∈ A ×ˢ B} : Set G) ≤ #A * #B :=
  by
  simp only [ex124', Set.mem_prod, exists_prop, Prod.exists]
  calc
    (#{x | ∃ a b : G, (a ∈ A ∧ b ∈ B) ∧ a * b = x}) =
        (#(⋃ (a ∈ A) (b ∈ B), ({(a : G) * (b : G)} : Set G))) :=
      ?_
  ...
  · congr 2
    ext x
    simp [Set.setOf_exists, Set.setOf_and, ← exists_and_left, and_assoc']
  · stop
    simp only [Set.iUnion_coe_set]
    congr
  ·
    exact
      cardinal.mk_Union_le_sum_mk.trans
        (Cardinal.sum_le_sum _ _ fun a => Cardinal.mk_iUnion_le_sum_mk)
  · simp_rw [Cardinal.mk_singleton, Cardinal.sum_const', mul_one]

/- ./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr * »(g, x)) | x «expr ∈ » «expr ∩ »(A, B)} -/
/- ./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr * »(g, x)) | x «expr ∈ » A} -/
/- ./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr * »(g, x)) | x «expr ∈ » B} -/
theorem ex_1_2_5 {G : Type _} [Group G] (A B : Set G) (g : G) :
...
theorem ex_1_2_6 {G : Type _} [Fintype G] [Group G] (A B : Subgroup G) (h : A ≤ B) :
    Subgroup.index A = Subgroup.relindex A B * Subgroup.index B :=
  (Subgroup.relindex_mul_index h).symm

theorem ex_1_2_7 {G : Type _} [Group G] (A B : Subgroup G) (h : A ≤ B) :
    Subgroup.index A = Subgroup.relindex A B * Subgroup.index B :=
  (Subgroup.relindex_mul_index h).symm

def d1214 (G : Type _) [Group G] (g : G) (n : ℤ) :=
  g ^ n

theorem l1_2_15_i (G : Type _) [Group G] (g : G) (m n : ℤ) : g ^ (m + n) = g ^ m * g ^ n :=
  zpow_add g m n

theorem l1_2_15_i' (G : Type _) [Group G] (g : G) (m n : ℤ) : (g ^ m) ^ n = g ^ (m * n) :=
  (zpow_mul g m n).symm

/- ./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr ^ »(g, k)) | k : exprℤ()} -/
theorem l1_2_15_ii (G : Type _) [Group G] (g : G) (m n : ℤ) :
    (Subgroup.closure ({g} : Set G) : Set G) = { (g ^ k) | k : ℤ } :=
  by
  ext
  simp only [SetLike.mem_coe, Set.mem_setOf_eq, Subgroup.mem_closure_singleton]

theorem l1_2_17 (G : Type _) [Group G] (g a b : G) (m n : ℤ)
...
theorem l1_2_19 (G : Type _) [Group G] (g : G) (d : ℤ) : g ^ d = 1 ↔ (orderOf g : ℤ) ∣ d :=
  orderOf_dvd_iff_zpow_eq_one.symm

theorem l1_2_20_i (G : Type _) [Group G] (g : G) (i j : ℤ) (h : IsOfFinOrder g) :
    g ^ i = g ^ j ↔ i ≡ j [ZMOD orderOf g] :=
  zpow_eq_zpow_iff_modEq

namespace Subgroup

theorem mem_closure_singleton' {G : Type _} [Group G] {x y : G} (h : IsOfFinOrder x) :
    y ∈ closure ({x} : Set G) ↔ ∃ n < orderOf x, x ^ n = y :=
  by
  simp only [← zpowers_eq_closure, exists_prop]
  --  rw mem_zpowers_iff_mem_range_order_of,
  constructor <;>
  rintro ⟨i, hi⟩
  · suffices ∃ n : ℤ, 0 ≤ n ∧ n < orderOf x ∧ x ^ n = y
      by
...
/- ./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr ^ »(g, i)) | i «expr ∈ » set.Ico[set.Ico] 0 (order_of[order_of] g)} -/
theorem l1_2_20_i' (G : Type _) [Group G] (g : G) (h : IsOfFinOrder g) :
    (Subgroup.closure ({g} : Set G) : Set G) =
      "./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr ^ »(g, i)) | i «expr ∈ » set.Ico[set.Ico] 0 (order_of[order_of] g)}" :=
  by ext; simp [Subgroup.mem_closure_singleton' h]

theorem l1_2_20_ii (G : Type _) [Group G] (g : G) (i j : ℤ) (h : ¬IsOfFinOrder g) :
    g ^ i = g ^ j ↔ i = j := by
  rw [← orderOf_eq_zero_iff] at h
  rw [zpow_eq_zpow_iff_modEq, h, Int.ofNat_zero, Int.modEq_zero_iff]

/- ./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr ^ »(g, i)) | i : exprℤ()} -/
theorem l1_2_20_ii' (G : Type _) [Group G] (g : G) (h : ¬IsOfFinOrder g) :
    (Subgroup.closure ({g} : Set G) : Set G) =
      "./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr ^ »(g, i)) | i : exprℤ()}" :=
  by
...
theorem zpow_inj_iff_of_orderOf_eq_zero (h : orderOf x = 0) {n m : ℤ} : x ^ n = x ^ m ↔ n = m :=
  by
  rw [orderOf_eq_zero_iff] at h
...
theorem l1_2_20_iii' (G : Type _) [Group G] (g : G) (h : ¬IsOfFinOrder g) :
  ...
  rw [l1_2_20_ii' _ _ h]
  rw [← orderOf_eq_zero_iff] at h
  have : ((Set.univ : Set ℕ).image fun i => g ^ i).Infinite :=
    by
    simp
...
theorem l1_2_21 (G : Type _) [Group G] [Fintype G] (g : G) : orderOf g ∣ Fintype.card G :=
  orderOf_dvd_card

theorem l1_2_21' (G : Type _) [Group G] [Fintype G] (g : G) : g ^ Fintype.card G = 1 :=
  orderOf_dvd_iff_pow_eq_one.mp orderOf_dvd_card

namespace Int

theorem gcd_val (a b : ℤ) : gcd a b = gcd (b % a) a := by
  rw [← nat_abs_euclidean_domain_gcd, EuclideanDomain.gcd_val, nat_abs_euclidean_domain_gcd]

end Int

theorem coe_hMul_inv_eq_one'' {n : ℕ} (x : Int) (h : IsCoprime x n) : (x * x⁻¹ : ZMod n) = 1 :=
  by
  cases n
  · simp [isCoprime_zero_right, -IsCoprime] at h
    rw [ZMod.mul_inv_eq_gcd, Nat.gcd_zero_right, Nat.cast_eq_one, ZMod.val, Int.cast_id,
      Int.IsUnit.natAbs_eq h]
  · haveI := n.succ_pos
...
def unitOfIsCoprime' {n : ℕ} (x : ℤ) (h : IsCoprime x n) : (ZMod n)ˣ :=
  ⟨x, x⁻¹, coe_hMul_inv_eq_one'' x h, by rw [mul_comm, coe_hMul_inv_eq_one'' x h]⟩

@[simp]
theorem coe_unit_of_isCoprime {n : ℕ} (x : ℤ) (h : IsCoprime x n) :
    (unitOfIsCoprime' x h : ZMod n) = x :=
  rfl

@[simp]
theorem ZMod.pow_totient' {n : ℕ} (x : (ZMod n)ˣ) : x ^ Nat.totient n = 1 :=
  by
  cases n
  · simp
...
theorem l1_2_23 (n : ℕ+) (x : ℤ) (h : IsCoprime x n) : x ^ Nat.totient n ≡ 1 [ZMOD n] := by
  rw [coe_coe n, ← CharP.intCast_eq_intCast (ZMod n), Int.cast_pow, Int.cast_one, ←
    coe_unit_of_isCoprime _ h, ← Units.val_pow_eq_pow_val, Units.val_eq_one, ← ZMod.pow_totient' _]

noncomputable def d1224 (G : Type _) [Group G] : ℕ :=
  Monoid.exponent G

theorem d1224_dvd {G : Type _} [Group G] (g : G) : orderOf g ∣ d1224 G :=
  Monoid.order_dvd_exponent g

theorem d1224_dvd' {G : Type _} [Group G] (n : ℕ) (h : ∀ g : G, orderOf g ∣ n) : d1224 G ∣ n :=
  Monoid.exponent_dvd_of_forall_pow_eq_one G n <|
    forall_imp (fun g => orderOf_dvd_iff_pow_eq_one.mp) h

theorem commute_of_order_dvd_two {G : Type _} [Group G] {a b : G} (h : ∀ g : G, orderOf g ∣ 2) :
    a * b = b * a := by
  simp_rw [orderOf_dvd_iff_pow_eq_one] at h
  rw [← mul_right_inj a, ← mul_left_inj b]
  calc
    a * (a * b) * b = a ^ 2 * b ^ 2 := by group
    _ = 1 := by rw [h, h, mul_one]
    _ = (a * b) ^ 2 := by rw [h]
    _ = a * (b * a) * b := by simp only [sq]; group

example {G : Type _} [Group G] (h : Monoid.exponent G = 2) (a b : G) : a * b = b * a :=
...
@[simp]
theorem coe_int_inv_num (x : ℤ) : (x : ℚ)⁻¹.num = Int.sign x := by
  simp [inv_def', div_num_denom, num_mk]

@[simp]
theorem coe_int_inv_den (x : ℤ) : (x : ℚ)⁻¹.den = if x = 0 then 1 else x.natAbs := by
  simp [inv_def', div_num_denom, denom_mk]

end Rat

theorem n1_2_24 :
    ∃ (G : Type) (_ : AddGroup G), (∀ g : G, IsOfFinAddOrder g) ∧ AddMonoid.exponent G = 0 :=
  by
  refine' ⟨ℚ ⧸ AddSubgroup.zmultiples (1 : ℚ), inferInstance, fun g => _, _⟩
  · induction g using QuotientAddGroup.induction_on'
...
theorem n1_2_24' {G : Type _} [Group G] [Fintype G] : d1224 G ≠ 0 :=
  Monoid.exponent_ne_zero_of_finite

theorem l1_2_25 {G : Type _} [Group G] [Fintype G] : d1224 G ∣ Fintype.card G :=
  Monoid.exponent_dvd_of_forall_pow_eq_one G (Fintype.card G) <| l1_2_21' G

theorem l1_2_25' {G : Type _} [Group G] : d1224 G ∣ Nat.card G :=
  by
...
noncomputable theorem d1226Ii {G : Type _} [Group G] (g h : G) : G :=
  ⁅g, h⁆

theorem n1_2_26_ii {G : Type _} [Group G] (g h : G) : ⁅g, h⁆ = ⁅h, g⁆⁻¹ := by group

theorem n1_2_26_ii' {G : Type _} [Group G] (g h : G) : ⁅g, h⁆ = 1 ↔ g * h = h * g := by
  rw [commutatorElement_def, mul_inv_eq_iff_eq_mul, mul_inv_eq_iff_eq_mul, one_mul]

/- ./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr⁅ , ⁆»(g, h)) | (g : G) (h : G)} -/
def d1226Iii (G : Type _) [Group G] : Subgroup G :=
  Subgroup.closure
    "./././Mathport/Syntax/Translate/Expr.lean:373:4: unsupported set replacement {(«expr⁅ , ⁆»(g, h)) | (g : G) (h : G)}"

theorem l1_2_26_iii' {G : Type _} [Group G] :
    (d1226Iii G : Set G) = {1} ↔ ∀ g h : G, g * h = h * g :=
  by
  simp only [d1226Iii]
  rw [Set.ext_iff]
  simp_rw [← n1_2_26_ii']
...
theorem l1_2_26_iii'' {G : Type _} [CommGroup G] : (d1226Iii G : Set G) = {1} :=
  l1_2_26_iii'.mpr mul_comm

def l1226Iii''' {G : Type _} [Group G] (h : (d1226Iii G : Set G) = {1}) : CommGroup G :=
  { show Group G by infer_instance with mul_comm := l1_2_26_iii'.mp h }

theorem ex_1_2_8 {G : Type _} [Group G] (h : ∀ g : G, g = 1 ∨ orderOf g = 2) (a b : G) :
    a * b = b * a := by
...
theorem ex_1_2_9 {G : Type _} [Group G] [Fintype G] (A B : Subgroup G)
    (h : gcd (Monoid.exponent A) (Monoid.exponent B) = 1) : (A : Set G) ∩ B = {1} :=
  by
...
example : orderOf c[(1 : Fin 4), 3, 2] ≠ 2 := by
  ...
  have : (Equiv.swap (1 : Fin 4) 3 * Equiv.swap 3 2 * (Equiv.swap 1 3 * Equiv.swap 3 2)) 1 = 1 := by
